...
export class WatcherMigration1784716748000 implements MigrationInterface {
  name = 'WatcherMigration1784716748000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DELETE FROM observation_status_entity where observationId in (SELECT id FROM observation_entity WHERE height > ${height} AND fromChain = 'cardano')`
    );
    await queryRunner.query(
