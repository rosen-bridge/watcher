...
const permitSecret = wasm.SecretKey.dlog_from_bytes(hexStrToUint8Array("ed5242e13db8161b2079d4cb49cb9553d87e90b0898e44bc10b9b5d309dbac65"));
    ...
    describe("getPermit", () => {
        ...
        it("checks get permit transaction is signed", async () => {
            ...
            chai.spy.on(ErgoNetwork, "getBoxWithToken", (address, tokenId) => {
                console.log(tokenId)
                if (tokenId === "a2a6c892c38d508a659caf857dbe29da4343371e597efd42e40f9bc99099a516") return wasm.ErgoBox.from_json(mockedResponseBody.watcherBox)
                else
                    throw Error("No box with token")
            })
    ...
    describe("returnPermit", () => {
        ...
        it("checks transaction is signed", async () => {
            ...
            chai.spy.on(boxes, "getPermits", () => {
                return [
                    wasm.ErgoBox.from_json(mockedResponseBody.permitBox),
                ]
            })
            chai.spy.on(boxes, "getRepoBox", () => {
            ...
            chai.spy.on(ErgoNetwork, "getBoxWithToken", (address, tokenId) => {
                if (tokenId === "6572676f") throw Error("There is no box with token id")
                return wasm.ErgoBox.from_json(mockedResponseBody.watcherBoxWithWIDToken)
            })
            ...
            expect(res.response).to.be.equal("5d2926187ccd2feb6ef526d7e6cd0efffe23c33b3fec26ab918cff75b7089fe5");
        });
        ...
        it("it checks case that the return permit transaction have permit box in its output", async () => {
            ...
            expect(res.response).to.be.equal("4e4ae9e870aefd70da618f55d27ea4b1040d58824eeaee841fa4cb83b6730003")
        });
