...
const permitBoxGenerator = (permitBoxValue: string, tokensList: Array<string>, WID: string) => {
    ...
    const blockHeaders = wasm.BlockHeaders.from_json(mockedResponseBody.last10BlockHeaders);
    ...
    return Buffer.from(signedTx.outputs().get(0).sigma_serialize_bytes()).toString('base64')
}
